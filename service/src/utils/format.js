...
async function queryCommentsCount(articles) {
  for (let article of articles) {
    const articleId = article.articleId
    let total = 0
    const { count } = await CommunityComment.findAndCountAll({
      limit: 1,
      offset: 0,
      where: { articleId }
    })
    total += count
    const { count: count2 } = await CommunityCommentReply.findAndCountAll({
      limit: 1,
      offset: 0,
      where: { articleId }
    })
...
async function queryNotificationUserAndCommentInfo(notifications) {
  for (let notif of notifications) {
    ...
    if (!replyCommentId) {
      // 说明是发表评论 不是回复 查文章信息
      const title = await Community.findOne({
        attributes: ['title', 'createTime'],
        where: { articleId }
      })
      ...
      if (notif.replyCommentLevel == 2) {
        content = await CommunityCommentReply.findOne({
          attributes: ['content', 'createTime'],
          where: { commentId: replyCommentId }
        })
      } else {
        content = await CommunityComment.findOne({
          attributes: ['content', 'createTime'],
          where: { commentId: replyCommentId }
        })
      }
    ...
    // 被回复者的信息
    info.replyUserInfo = await User.findOne({
      attributes: { exclude: ['password'] },
      where: { uid: replyUserId }
    })
    // 回复者的信息
    info.commentUserInfo = await User.findOne({
      attributes: { exclude: ['password'] },
      where: { uid: authorId }
    })
    // 回复内容
    let commentContent
    if (notif.level == 2) {
      commentContent = await CommunityCommentReply.findOne({
        attributes: ['content', 'createTime'],
        where: { commentId: currentCommentId }
      })
    } else {
      commentContent = await CommunityComment.findOne({
        attributes: ['content', 'createTime'],
        where: { commentId: currentCommentId }
      })
    }
    ...
    notif.dataValues = Object.assign(info, {
      read: notif.read,
      articleId,
