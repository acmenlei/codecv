...
## 提交 Pull Request
1. Fork [此仓库](https://github.com/Acmenlei/markdown-resume-to-pdf)，从 `master` 创建分支。新功能实现请发 pull request 到 `feature` 分支。其他更改发到 `master` 分支。
2. 使用 `npm install pnpm -g` 安装 `pnpm` 工具。
3. 根目录下执行 `pnpm dev`。
4. `/service/` 目录下执行 `pnpm install`。
